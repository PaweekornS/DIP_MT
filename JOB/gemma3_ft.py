...
vectorstore = FAISS.load_local(f"{ROOT_DIR}/vector/{retriever}", embeddings, allow_dangerous_deserialization=True)
...
def formatting_prompt(df, tokenizer):    
    batch = []
    for _, row in tqdm(df.iterrows(), total=len(df), leave=True):
        prompt = [
            {"role": "user", "content": instruction.format(
                WIPO=row['WIPO'],
                RAG_DOC=get_relevant_docs(row["ENG"], 4),
                ENGLISH=row["ENG"])},
